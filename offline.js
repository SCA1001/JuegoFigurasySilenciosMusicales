{
	"version": 1601266105,
	"fileList": [
		"data.js",
		"c2runtime.js",
		"jquery-3.4.1.min.js",
		"offlineClient.js",
		"images/sprite-sheet0.png",
		"images/sprite2-sheet0.png",
		"images/sprite3-sheet0.png",
		"images/tiledbackground.png",
		"images/tiledbackground2.png",
		"images/target-sheet0.png",
		"images/target-sheet1.png",
		"images/sprite5-sheet0.png",
		"images/sprite5-sheet1.png",
		"images/sprite5-sheet2.png",
		"images/sprite6-sheet0.png",
		"images/tiledbackground3.png",
		"images/sprite7-sheet0.png",
		"images/sprite8-sheet0.png",
		"images/tiledbackground4.png",
		"images/tiledbackground5.png",
		"images/sprite9-sheet0.png",
		"images/tiledbackground6.png",
		"images/indicador-sheet0.png",
		"images/indicador-sheet1.png",
		"images/nombres-sheet0.png",
		"images/sprite10-sheet0.png",
		"images/sprite11-sheet0.png",
		"images/sprite12-sheet0.png",
		"images/remolque-sheet0.png",
		"images/tiledbackground7.png",
		"images/sprite13-sheet0.png",
		"images/tiledbackground8.png",
		"images/sprite4-sheet0.png",
		"images/sprite14-sheet0.png",
		"images/sprite16-sheet0.png",
		"images/tiledbackground9.png",
		"images/sprite17-sheet0.png",
		"images/sprite18-sheet0.png",
		"images/sprite19-sheet0.png",
		"images/soltarbtn-sheet0.png",
		"images/faded-sheet0.png",
		"images/sprite20-sheet0.png",
		"images/sprite21-sheet0.png",
		"media/golpemetalico.ogg",
		"media/tinmetal.ogg",
		"media/factory ambient sound effect-grqu8ozfo1o.ogg",
		"media/maquinamueve.ogg",
		"media/cantasometal.ogg",
		"media/wrong answer _ britain's got talent buzzer sound fx.ogg",
		"media/afrola - bird creek (no copyright music)-eqfjyfohffe.ogg",
		"icon-16.png",
		"icon-32.png",
		"icon-114.png",
		"icon-128.png",
		"icon-256.png",
		"loading-logo.png"
	]
}